...
def r_fname(args, cat_i, search_for, sub, illegal_chars):
    ...
    if args.subreddit:
        if cat_i == global_vars.short_cat[5]:
            ...
            raw_n = str(("r-%s-%s %s") % 
                (sub, global_vars.categories[global_vars.short_cat.index(cat_i)], global_vars.date))
            fname = fix(raw_n, illegal_chars)
    elif args.basic:
        if cat_i == 5:
            raw_n = str(("r-%s-%s-'%s' %s") % 
                (sub, global_vars.categories[cat_i], search_for, global_vars.date))
            fname = fix(raw_n, illegal_chars)
        else:
            raw_n = str(("r-%s-%s %s") % 
                (sub, global_vars.categories[cat_i], global_vars.date))
            fname = fix(raw_n, illegal_chars)
